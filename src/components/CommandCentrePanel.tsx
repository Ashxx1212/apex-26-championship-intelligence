...
function PulseMetric({
  ...
  const content = (
    <>
      ...
      {actionLabel && (
        <span className="mt-3 block text-[8px] tracking-[0.14em] text-cyan/0 transition-colors group-hover:text-cyan/65">
          {actionLabel}
        </span>
      )}
    </>
  ...
  if (onClick) {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`group relative overflow-hidden border bg-black/[0.18] px-4 py-4 text-left transition-colors duration-200 hover:border-cyan/35 ${
          isSelected
            ? 'border-cyan/45 bg-cyan/[0.06]'
            : 'border-white/[0.09]'
        } ${activeAccent.glow}`}
      >
        {content}
...
function DriverSignal({
  ...
  return (
    <button
      ...
      className={`group relative w-full overflow-hidden border px-3 py-3 text-left transition-colors duration-200 ${
        isSelected
          ? 'border-cyan/45 bg-cyan/[0.07] shadow-[0_0_18px_rgba(0,212,255,0.10)]'
          : 'border-white/[0.09] bg-black/[0.20] hover:border-cyan/25'
      }`}
    ...
    >
      ...
      <span className="mt-2 block text-[8px] tracking-[0.14em] text-cyan/0 transition-colors group-hover:text-cyan/65">
        OPEN DRIVER INTEL
      </span>
    </button>
  );
...
export function CommandCentrePanel({
  ...
  const timingEntries = useMemo(() => {
    ...
    return Array.from(latestResult.driverResults.entries())
      ...
      .map(([driverNumber, result]) => {
        ...
        return {
          driverNumber,
          driverName: driver?.full_name || driver?.broadcast_name || 'Unknown driver',
  ...
  const decisionEvent =
    selectedContextWeekend ?? focusWeekend ?? latestWeekend ?? null;

  const decisionEventLabel = selectedContextWeekend
  ...
  const archiveIndexed =
    data?.analyticsCoverage.indexedRaceResults ?? 0;

  const archiveCompleted =
    data?.analyticsCoverage.totalCompletedRaceSessions ?? 0;

  const archiveHasPendingWork =
    data?.analyticsArchive.hasPendingWork ?? false;

  const championshipBrief =
    leader && second
      ? `${leader.driverAcronym} leads ${second.driverAcronym} by ${
          second.gapToLeader ?? 0
        } points.`
      : leader
        ? `${leader.driverAcronym} leads the current verified standings.`
        : 'Championship standings are awaiting verified ingestion.';

  const latestVerifiedBrief = latestMeeting
    ? latestWeekend?.raceWinner
      ? `${latestWeekend.raceWinner.driverAcronym} won for ${latestWeekend.raceWinner.teamName}.`
      : 'Winner record is awaiting verification.'
    : 'No completed race result is currently indexed.';

  const archiveBrief =
    archiveCompleted > 0
      ? archiveHasPendingWork
        ? `${archiveIndexed}/${archiveCompleted} completed race records indexed.`
        : `${archiveIndexed}/${archiveCompleted} completed race records verified.`
      : 'Archive coverage is awaiting completed-race ingestion.';
        const closestChallenger = second ?? null;

  const archiveOutstanding = Math.max(
    0,
    archiveCompleted - archiveIndexed
  );

  const nextCalendarWeekend =
    data?.raceWeekends.find(
      (weekend) => weekend.meetingKey === nextMeeting?.meeting_key
    ) ?? null;

  const operationalWatchWeekend = currentMeeting
    ? focusWeekend
    : nextCalendarWeekend ?? focusWeekend ?? null;

  const operationalWatchLabel = currentMeeting
    ? 'ACTIVE EVENT WINDOW'
    : nextMeeting
      ? 'NEXT EVENT WINDOW'
      : 'CALENDAR WATCH';

  const operationalWatchDetail = operationalWatchWeekend
    ? `${operationalWatchWeekend.circuitShortName} · ${
        operationalWatchWeekend.country
      } · ${formatEventDate(operationalWatchWeekend.dateStart)}`
    : 'No verified calendar window is currently indexed.';
  if (isLoading && !data) {
    return (
  ...
  return (
    <section className="space-y-4">
      {/* Command Deck */}
      <div className="relative overflow-hidden apex-live-scan border border-white/[0.10] bg-[#0a0c10]/90 shadow-[0_22px_60px_rgba(0,0,0,0.28)]">
          ...
          <div className="relative flex flex-col justify-between overflow-hidden p-5 md:p-7 lg:p-8">
            ...
            <div className="relative">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span
                  className={`border px-2.5 py-1 text-[9px] font-semibold tracking-[0.16em] uppercase ${
                    meetingType === 'active' ? 'apex-active-weekend' : ''
                  } ${getWeekendStatusClasses(meetingType)}`}
                >
              ...
              <div className="border-b border-white/[0.10] px-5 py-5">
                ...
                <div className="mt-5 flex items-start gap-3">
                  <div className="relative mt-1 h-2.5 w-2.5 shrink-0">
                    <div className={`absolute inset-0 rounded-full ${dataState.dot} apex-data-core shadow-[0_0_12px_rgba(0,255,255,0.7)]`} />
  ...
  Live Refresh Control
            ...
            <section className="relative overflow-hidden border border-white/[0.10] bg-[#0b0d12]/80 shadow-[0_18px_45px_rgba(0,0,0,0.18)]">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_8%_0%,rgba(220,20,60,0.07),transparent_25%),radial-gradient(circle_at_92%_100%,rgba(0,212,255,0.07),transparent_28%)]" />
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[8px] tracking-[0.14em] text-white/30 uppercase">
                Immediate operating brief from the verified snapshot
              </p>
            </div>
          </div>

          <span className="border border-cyan/20 bg-cyan/[0.04] px-2.5 py-1 text-[8px] font-semibold tracking-[0.14em] text-cyan">
            VERIFIED INTELLIGENCE ONLY
          </span>
        </div>

        <div className="relative grid grid-cols-1 gap-px bg-white/[0.08] md:grid-cols-2 xl:grid-cols-4">
          <div className="min-h-[154px] bg-[#0d1015] p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Trophy className="h-3.5 w-3.5 text-crimson" />
                <p className="text-[8px] tracking-[0.16em] text-white/35 uppercase">
                  Championship Pressure
                </p>
              </div>

              <span className="text-[8px] tracking-[0.12em] text-crimson">
                LIVE TABLE
              </span>
            </div>

            <p className="mt-5 text-2xl font-black tracking-[0.08em] text-white">
              {leader?.driverAcronym || '—'}
            </p>
            ...
            {leader && (
              <button
                type="button"
                onClick={() => onDriverSelect?.(leader.driverNumber)}
                className="mt-4 text-[8px] font-semibold tracking-[0.13em] text-cyan transition-colors hover:text-white"
              >
                OPEN LEADER INTEL →
              </button>
            )}
          ...
          <button
            ...
            onClick={() => {
              if (latestWeekend) {
                onMeetingSelect?.(latestWeekend.meetingKey);
              }
            }}
            disabled={!latestWeekend}
            className="group min-h-[154px] bg-[#0d1015] p-4 text-left transition-colors hover:bg-cyan/[0.035] disabled:cursor-default"
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <ShieldCheck className="h-3.5 w-3.5 text-green-400" />
              ...
              <span className="text-[8px] tracking-[0.12em] text-green-400">
                RECORD
              </span>
            </div>

            <p className="mt-5 truncate text-lg font-black tracking-[0.04em] text-white">
              {latestMeeting?.meeting_name || 'No result indexed'}
            </p>
            ...
            {latestWeekend && (
              <span className="mt-4 block text-[8px] font-semibold tracking-[0.13em] text-cyan/70 transition-colors group-hover:text-cyan">
                OPEN CIRCUIT BRIEF →
              </span>
            )}
          </button>
          ...
          <div className="min-h-[154px] bg-[#0d1015] p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span
                className={`text-[8px] tracking-[0.12em] ${
                  archiveHasPendingWork ? 'text-amber' : 'text-green-400'
                }`}
              >
                {archiveHasPendingWork ? 'PARTIAL' : 'COMPLETE'}
              </span>
            </div>

            <p
              className={`mt-5 text-2xl font-black tracking-[0.08em] ${
                archiveHasPendingWork ? 'text-amber' : 'text-green-400'
              }`}
            >
              {archiveIndexed}/{archiveCompleted || '—'}
            </p>
            ...
            <p className="mt-4 text-[8px] tracking-[0.13em] text-white/30">
              DATA INTEGRITY CONTROLS BELOW
            </p>
          </div>
          ...
          <button
            ...
            onClick={() => {
              if (decisionEvent) {
                onMeetingSelect?.(decisionEvent.meetingKey);
              }
            }}
            disabled={!decisionEvent}
            className="group min-h-[154px] bg-[#0d1015] p-4 text-left transition-colors hover:bg-amber/[0.035] disabled:cursor-default"
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <CalendarDays className="h-3.5 w-3.5 text-cyan" />
              ...
              <span
                className={`text-[8px] tracking-[0.12em] ${
                  decisionEvent?.status === 'active'
                    ? 'text-amber'
                    : decisionEvent?.status === 'completed'
                      ? 'text-green-400'
                      : 'text-cyan'
                }`}
              >
                {decisionEvent?.status?.toUpperCase() || 'PENDING'}
              </span>
            </div>

            <p className="mt-5 truncate text-lg font-black tracking-[0.04em] text-white">
              {decisionEvent?.meetingName || 'Calendar focus pending'}
            </p>

            <p className="mt-2 text-[10px] leading-relaxed text-white/55">
              {decisionEvent
                ? `${decisionEventLabel} · ${decisionEvent.circuitShortName} · ${decisionEvent.country}`
                : 'No verified event context is currently available.'}
            </p>
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.10] px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[8px] tracking-[0.14em] text-white/30 uppercase">
                Priority signals requiring analyst attention
              </p>
            </div>
          </div>

          <span className="border border-amber/20 bg-amber/[0.04] px-2.5 py-1 text-[8px] font-semibold tracking-[0.14em] text-amber">
            3 PRIORITY SIGNALS
          </span>
        </div>

        <div className="relative grid grid-cols-1 gap-px bg-white/[0.08] lg:grid-cols-3">
          <button
            type="button"
            onClick={() => {
              if (closestChallenger) {
                onDriverSelect?.(closestChallenger.driverNumber);
              }
            }}
            disabled={!closestChallenger || !onDriverSelect}
            className="group min-h-[136px] bg-[#0d1015] p-4 text-left transition-colors hover:bg-crimson/[0.035] disabled:cursor-default"
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Trophy className="h-3.5 w-3.5 text-crimson" />
            ...
            <p className="mt-5 text-lg font-black tracking-[0.06em] text-white">
              {closestChallenger?.driverAcronym || '—'}
            </p>

            <p className="mt-2 text-[10px] leading-relaxed text-white/55">
              {leader && closestChallenger
                ? `${closestChallenger.driverAcronym} trails ${
                    leader.driverAcronym
                  } by ${closestChallenger.gapToLeader ?? '—'} points.`
                : 'The nearest championship challenger is awaiting verified standings.'}
            </p>
          ...
          <div className="min-h-[136px] bg-[#0d1015] p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Database className="h-3.5 w-3.5 text-amber" />
              ...
              <span
                className={`text-[8px] tracking-[0.12em] ${
                  archiveOutstanding > 0 ? 'text-amber' : 'text-green-400'
                }`}
              >
                {archiveOutstanding > 0 ? 'ACTION NEEDED' : 'CLEAR'}
              </span>
            </div>

            <p
              className={`mt-5 text-lg font-black tracking-[0.06em] ${
                archiveOutstanding > 0 ? 'text-amber' : 'text-green-400'
              }`}
            >
              {archiveOutstanding > 0
                ? `${archiveOutstanding} PENDING`
                : 'ARCHIVE CLEAR'}
            </p>

            <p className="mt-2 text-[10px] leading-relaxed text-white/55">
              {archiveOutstanding > 0
                ? `${archiveOutstanding} completed race ${
                    archiveOutstanding === 1 ? 'record remains' : 'records remain'
                  } outside the indexed archive.`
                : 'All completed race records are indexed in the verified archive.'}
            </p>

            <p className="mt-4 text-[8px] tracking-[0.13em] text-white/30">
              DATA INTEGRITY CONTROLS AVAILABLE BELOW
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={() => {
              if (operationalWatchWeekend) {
                onMeetingSelect?.(operationalWatchWeekend.meetingKey);
              }
            }}
            disabled={!operationalWatchWeekend || !onMeetingSelect}
            className="group min-h-[136px] bg-[#0d1015] p-4 text-left transition-colors hover:bg-cyan/[0.035] disabled:cursor-default"
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <TimerReset className="h-3.5 w-3.5 text-cyan" />

                <p className="text-[8px] tracking-[0.16em] text-white/35 uppercase">
                  {operationalWatchLabel}
                </p>
              </div>

              <span
                className={`text-[8px] tracking-[0.12em] ${
                  operationalWatchWeekend?.status === 'active'
                    ? 'text-amber'
                    : 'text-cyan'
                }`}
              >
                {operationalWatchWeekend?.status?.toUpperCase() || 'PENDING'}
              </span>
            </div>

            <p className="mt-5 truncate text-lg font-black tracking-[0.04em] text-white">
              {operationalWatchWeekend?.meetingName || 'Calendar pending'}
            </p>

            <p className="mt-2 text-[10px] leading-relaxed text-white/55">
              {operationalWatchDetail}
            </p>

            {operationalWatchWeekend && (
              <span className="mt-4 block text-[8px] font-semibold tracking-[0.13em] text-cyan/70 transition-colors group-hover:text-cyan">
                OPEN EVENT INTELLIGENCE →
              </span>
            )}
...
/>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.3fr_0.7fr]">
        <div className="overflow-hidden border border-white/[0.10] bg-[#0b0d12]/75">
          ...
          <div className="p-4 md:p-5">
            ...
            {timingEntries.length > 0 ? (
              <div className="grid gap-2 apex-stagger">
                {timingEntries.map((entry, index) => (
                  <button
                    type="button"
                    key={`${entry.position}-${entry.acronym}`}
                    onClick={() => onDriverSelect?.(entry.driverNumber)}
                    title={`Open ${entry.driverName} in Driver Intel`}
                    className={`group flex w-full items-center gap-4 border px-4 py-3 text-left transition-colors duration-200 ${
                      selectedDriverNumber === entry.driverNumber
                        ? 'border-cyan/45 bg-cyan/[0.07] shadow-[0_0_18px_rgba(0,212,255,0.10)]'
                        : index === 0
                          ? 'border-cyan/20 bg-cyan/[0.025] hover:border-cyan/40'
        ...
        <div className="overflow-hidden border border-white/[0.10] bg-[#0b0d12]/75">
          ...
          <div className="divide-y divide-white/[0.08] apex-stagger">
  ...
  {archiveCompleted} VERIFIED RACE RECORDS INDEXED
        ...
        <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-3">
          <PulseMetric
            ...
            accent="crimson"
            isSelected={selectedDriverNumber === leader?.driverNumber}
            onClick={
              leader
                ? () => onDriverSelect?.(leader.driverNumber)
                : undefined
            }
            actionLabel={leader ? 'OPEN DRIVER INTEL' : undefined}
          />
          ...
          <PulseMetric
            ...
            accent="amber"
            isSelected={selectedDriverNumber === second?.driverNumber}
            onClick={
              second
                ? () => onDriverSelect?.(second.driverNumber)
                : undefined
            }
            actionLabel={second ? 'OPEN DRIVER INTEL' : undefined}
          />
          ...
          <PulseMetric
            ...
            accent="cyan"
            isSelected={selectedTeamName === constructorLeader?.teamName}
            onClick={
              constructorLeader
                ? () => onTeamSelect?.(constructorLeader.teamName)
                : undefined
            }
            actionLabel={constructorLeader ? 'OPEN TEAM PERFORMANCE' : undefined}
          />
        </div>
      </div>
  ...
  source records pending
      ...
      <div className="relative overflow-hidden border border-white/[0.10] bg-[#0b0d12]/75">
        ...
        <div className="relative grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 xl:grid-cols-4 apex-stagger">
          {radarRounds.length > 0 ? (
            radarRounds.map((weekend, index) => (
              <button
                type="button"
                key={weekend.meetingKey}
                onClick={() => onMeetingSelect?.(weekend.meetingKey)}
                title={`Open ${weekend.meetingName} in Circuit Matrix`}
                className={`group relative overflow-hidden border p-4 text-left transition-colors duration-200 hover:border-cyan/30 ${
                  selectedMeetingKey === weekend.meetingKey
                    ? 'border-cyan/45 bg-cyan/[0.06] shadow-[0_0_18px_rgba(0,212,255,0.10)]'
                    : getRoundStatusClasses(weekend.status)
                }`}
              >
                ...
                <span className="relative mt-3 block text-[8px] tracking-[0.14em] text-cyan/0 transition-colors group-hover:text-cyan/65">
                  OPEN CIRCUIT MATRIX
                </span>
              </button>
            ))
