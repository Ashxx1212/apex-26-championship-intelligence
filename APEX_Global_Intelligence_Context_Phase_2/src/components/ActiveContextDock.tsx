...
export function ActiveContextDock({
  ...
  const liveMeeting =
    ...
    (data?.currentMeeting
      ? data?.raceWeekends.find(
          (weekend) => weekend.meetingKey === data.currentMeeting?.meeting_key
        ) ?? null
      : null);

  const selectedMeetingIsDifferentFromLive =
    Boolean(
      selectedMeeting &&
        liveMeeting &&
        selectedMeeting.meetingKey !== liveMeeting.meetingKey
    );
        ...
        <div className="min-w-0 bg-[#091015] px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[8px] tracking-[0.14em] text-white/35 uppercase">
              {selectedMeetingKey !== null ? 'Selected Weekend' : 'Live Event Focus'}
            </p>
          </div>
          ...
          <p className="mt-1 text-[9px] text-white/40">
            {selectedMeeting
              ? selectedMeetingIsDifferentFromLive
                ? `LIVE NOW · ${liveMeeting?.meetingName}`
